...
const AiChatModule: React.FC<AiChatModuleProps> = ({ 
  ...
  // User List Component (Right Sidebar)
  const UserList = () => (
    <div className="bg-[#1f1f1f] h-full overflow-y-auto border-l border-gray-800 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent">
        <div className="p-2 border-b border-gray-800 bg-[#151515] sticky top-0 z-10">
            <h3 className="text-[#00ff9d] text-xs font-mono font-bold uppercase text-center tracking-widest">
        ...
        <ul className="p-2 space-y-1">
            {allUsersInRoom.map(user => (
                <li 
                    ...
                    className={`
                        group flex items-center gap-2 p-1.5 rounded cursor-pointer transition-colors
                        ${user.id === currentUser.id ? 'bg-[#00ff9d]/10' : 'hover:bg-white/5'}
                    `}
                >
                    <div className="relative shrink-0">
                        <img src={user.avatar} className="w-8 h-8 rounded bg-gray-800 object-cover" />
                        <div className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-[#1f1f1f] ${user.isBot ? 'bg-blue-500' : 'bg-[#00ff9d]'}`}></div>
                    </div>
                    <div className="min-w-0 overflow-hidden">
                        <div className={`text-sm truncate font-mono ${user.isBot ? 'text-blue-400' : 'text-gray-300'}`}>
                            {user.isBot && '@'}{user.name}
                        </div>
                        {user.isBot && <div className="text-[10px] text-gray-500 truncate">{user.role}</div>}
                    </div>
                </li>
            ))}
        </ul>
  ...
  return (
    <div className="flex h-full w-full bg-[#252525]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
         ...
         {/* Messages */}
         <div className="flex-1 overflow-y-auto p-4 space-y-3 relative bg-[#252525]">
            {messages.map((msg) => {
                ...
                return (
                    <div key={msg.id} className={`flex gap-3 ${isMe ? 'flex-row-reverse' : ''} group`}>
                        <div className="shrink-0 mt-0.5">
                            <img src={msg.senderAvatar} className="w-8 h-8 rounded object-cover bg-gray-800" />
                        </div>
                        <div className={`max-w-[85%] md:max-w-[70%]`}>
                             <div className={`flex items-baseline gap-2 mb-0.5 ${isMe ? 'justify-end' : ''}`}>
                                 <span className={`text-xs font-bold cursor-pointer hover:underline ${isMe ? 'text-[#00ff9d]' : 'text-blue-400'}`}>{msg.senderName}</span>
                                 <span className="text-[10px] text-gray-600">{msg.timestamp.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</span>
                             </div>
                             <div className={`p-2.5 rounded text-sm leading-relaxed whitespace-pre-wrap ${
                                 isMe 
                                 ? 'bg-[#00ff9d]/10 text-[#00ff9d] border border-[#00ff9d]/20' 
                                 : 'bg-[#151515] text-gray-300 border border-gray-800'
                             }`}>
                                {msg.image && <img src={msg.image} className="max-w-full h-40 object-contain rounded mb-2 border border-black/50" />}
                                {msg.text}
                             </div>
                        </div>
                    </div>
         ...
         {/* Input Area */}
         <div className="bg-[#1a1a1a] border-t border-gray-800 p-3 flex gap-2 shrink-0">
             {isPrivate && (
                 <>
             ...
             <form onSubmit={handleSendMessage} className="flex-1 flex gap-2">
                 <input 
                    ...
                    className="flex-1 bg-[#050505] text-gray-200 border border-gray-700 rounded px-4 focus:outline-none focus:border-[#00ff9d] placeholder-gray-600"
                    placeholder="Mesajınızı yazın..."
                 />
                 <button type="submit" className="px-6 bg-[#00ff9d] text-black font-bold rounded hover:bg-[#00cc7d] transition-colors">GÖNDER</button>
             </form>
         </div>
      ...
      {/* Desktop: Always visible. Mobile: Absolute over content if toggled */}
      <div className={`
          ${showMobileUsers ? 'absolute right-0 top-0 bottom-0 w-64 shadow-2xl z-50' : 'hidden md:block w-56 shrink-0'}
      `}>
          <UserList />
          ...
          {showMobileUsers && (
              <button 
                ...
                className="md:hidden absolute top-2 right-2 text-white bg-red-600 rounded-full w-6 h-6 flex items-center justify-center font-bold"
              >
                  ×
              </button>
          )}
